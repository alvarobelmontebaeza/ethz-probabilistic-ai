...
class BayesianLayer(torch.nn.Module):
    ...
    def __init__(self, input_dim, output_dim, bias=True):
        ...
        # Initialize layer parameters as indicated in Graves 2011
        self.prior_mu = 0.0
        self.prior_sigma = 0.075
        self.weight_mu = nn.Parameter(torch.Tensor(output_dim,input_dim).normal_(0, 0.1))
        self.weight_logsigma = nn.Parameter(torch.Tensor(output_dim, input_dim).normal_(0, 0.1))
    ...
    def forward(self, inputs):
        ...
        num_samples = 128
        weight = Normal(self.weight_mu,torch.exp(self.weight_logsigma)).rsample((num_samples,)).mean(0)

        if self.use_bias:
            # TODO: enter your code here
            bias = Normal(self.bias_mu,torch.exp(self.bias_logsigma)).rsample((num_samples,)).mean(0)
        else:
            bias = None
    ...
    def _kl_divergence(self, mu, logsigma):
        ...
        # TODO: enter your code here
        # Compute the KL Divergence by using pytorch's own function
        # return KL(q||p), where q = Gaussian posterior and p = Gaussian prior
        kl = torch.distributions.kl_divergence(Normal(mu,torch.exp(logsigma)),Normal(self.prior_mu,self.prior_sigma)).mean()
...
class BayesNet(torch.nn.Module):
    ...
    def predict_class_probs(self, x, num_forward_passes=2):
        assert x.shape[1] == 28**2
        batch_size = x.shape[0]


        # TODO: make n random forward passes
        probs = torch.zeros(size=(batch_size,10))

        for _ in range(num_forward_passes):
            # compute the categorical softmax probabilities
            # Insert temperature scaling for calibration
            probs += F.softmax(self.forward(x),dim=1)
    ...
    def kl_loss(self):
        ...
        '''
        kl_loss = torch.zeros(size=([]))

        # Iterate over each layer        
        for layer in self.net.children():
            if not isinstance(layer,BayesianLayer):
                kl_loss += layer[0].kl_divergence()
            else:
