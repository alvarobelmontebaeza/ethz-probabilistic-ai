...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            # Follow the current policy for a given state. Obtain distribution over actions and its log
            distribution, _ = self.pi(state)
            action = distribution.sample()
            logp_a = self.pi._log_prob_from_distribution(distribution,action)
            # Obtain value function for current state
            value = self.v(state)

        # Return values as numpy arrays
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        # Obtain undiscounted TD-Residual as r(s_t) + V(s_{t+1}) - V(s_t)
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            # Retrieve data from episodes
            data = buf.get()
            states = data['obs']
            actions = data['act']
            returns = data['ret']
            tdresiduals = data['tdres']
            #Do 1 policy gradient update
            pi_optimizer.zero_grad() #reset the gradient in the policy optimizer
            # Pass again states through actor and critic to compute gradients
            ...
            loss_pi = torch.sum(tdresiduals*(-logp)) / len(logp)
            loss_pi.backward()
            pi_optimizer.step()
            ...
            for _ in range(100):
                ...
                loss_v = (values - returns)**2
                loss_v = torch.sum(loss_v) / len(loss_v)
                loss_v.backward()
                v_optimizer.step()
    ...
    def get_action(self, obs):
        ...
        """
        # TODO: Implement this function.
        # Do a forward step through the AC and return the action sampled from the policy
